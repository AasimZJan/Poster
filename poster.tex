...
\end{document}
  ...
  \textbf{Model} \cite{2018ApJ...863L..41F}
%
  \begin{multline}
    \frac{\mathbb{d}(\mathrm{Rate})}{\mathbb{d}m_1\mathbb{d}m_2} \left( z \right) =
    \notag \\
    \mathrm{LocalRate} \times \mathrm{PopulationPDF}_{\text{(model A)}}(m_1, m_2; \alpha, \mmax) \times (1 + z)^\lambda
  \end{multline}
%
  \begin{columns}
    \centering
    \begin{column}{0.45\textwidth}
      \includegraphics[width=\textwidth]{paper_submodules/O2RandPPaper/figures/rate_v_z}
    \end{column}
    ~
    \begin{column}{0.45\textwidth}
    \includegraphics[width=\textwidth]{paper_submodules/O2RandPPaper/figures/corner_redshift}
    \end{column}
  \end{columns}

Results consistent with no evolution, but has slight preference towards increase with $z$.




\noindent \textbf{Parametric spin magnitude, tilt models}: Beta distribution for $a_i$  and Gaussian and/or mixture for $\hat{L}\cdot \hat{S}_i$ \cite{2018arXiv180506442W,2017PhRvD..96b3012T}

\noindent \textbf{Results}: Spin tilts uninformative, spin magnitudes not large.

\noindent \textbf{Model selection}: Using \emph{discrete} mass and spin
distributions, drill down probabilities for qualitatively chosen models \cite{2017Natur.548..426F,2018arXiv180901401T}
...
\begin{columns}
  ...
  \begin{column}[c]{0.45\textwidth}
    \centering
    \includegraphics[width=\textwidth]{img/spin_mag_parameter}
  \end{column}
  ~
  \begin{column}[c]{0.50\textwidth}
    \noindent \textbf{Log Bayes Factors}

    \begin{centering}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{|l|c|c|c|c|}
      \hline
      $p(a) \propto$ & $e^{-5a}$ & $1-a$ & $1$ & $a$ \\ \hline\hline
      Isotropic & 1.10 & 0.0 & -0.93 & -2.07 \\ \hline
      Restricted	& 3.39 & 3.26 & 1.31 & 0.11 \\ \hline
      Aligned & 1.58 & -4.12 & -12.92 & -32.37 \\ \hline
    \end{tabular}%
    }
    \end{centering}

    \emph{\tiny Note: Restricted $=$ isotropic without negative spins}
  \end{column}
\end{columns}
\noindent \textbf{Calibrating VT}: Sensitivity estimated from detailed end-to-end search test (``injections'').
VT model calibrated accurately to those injections.

\noindent \textbf{Spin dependence in VT?}: Did not matter.

\noindent \textbf{Waveforms}: Different waveforms produce different posteriors for some events and quantities (e.g., GW151226,
$\chi_{\rm eff}$). We checked our answers didn't change when we changed waveforms. 
  ...
  \begin{center}
    \begin{columns}
      \begin{column}[c]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/gw151226_chieff}
      \end{column}
      ~
      \begin{column}[c]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/vt_cal_simplified}
      \end{column}
    \end{columns}
    \vspace{-5em}
  \end{center}
...
% Rate
\noindent \textbf{Rate}: Robust merger rate $R
= \RATEimrpNoSpinVTQuadCpmU$ %, proposed population or spin dependence  not a significant systematic

\vskip20pt

\noindent \textbf{``Maximum mass''}: Rate  drops significantly for $m_1> 43 M_\odot$; no BHs seen above this

\noindent \textbf{Minimum mass}: No sensitivity to very small BHs ($m_1 < 5 \Msol{}$)

\noindent \textbf{Primary mass distribution}: May not be a pure power law;  modest preference for a model with a broad Gaussian at high mass.

\noindent \textbf{Favors equal mass ratio}: Preferentially predict $q\simeq 1$ ($\beta_q>0$)

\vskip20pt

\noindent \textbf{Lots of large, aligned spins?} No events like this!

\noindent \textbf{Spin magnitudes not preferentially large}: Even accounting for (very) uncertain orientation distributions

\noindent \textbf{Uninformative about orientation}: Spins are
  too small.  Can't tell if spins are random, aligned, or anything in between.


\vskip20pt
\noindent \textbf{Rate vs redshift}: Hints of trend with redshift


%% ASTRO IMPLICATIONS


\noindent \emph{Astrophysical implications?} (a) Begin to pin down upper mass limit possibly set by stellar physics; (b)
cluster vs field - even with small spins could be able to disentangle soon.